...
G2L["8"]["TextColor3"] = Color3.fromRGB(1,1,1);
...
G2L["a"]["TextColor3"] = Color3.fromRGB(1,1,1);
...
G2L["c"]["TextColor3"] = Color3.fromRGB(1, 1, 1);
...
G2L["e2"]["TextColor3"] = Color3.fromRGB(1, 1, 1);
...
G2L["f"]["TextColor3"] = Color3.fromRGB(1,1,1);
...
G2L["11"]["TextColor3"] = Color3.fromRGB(1, 1, 1);
		...
		remote.OnClientEvent:Connect(function(...)
			...
			for name, value in pairs(args) do
				...
				if type(newtonbar) == "table" then
					value = tabelhell(newtonbar, 2)
				end
				if type(value) == "userdata" then if not value:IsA("CFrame") then value = value:GetFullName() end end
				local ender = ","
				passes += 1
				if not tonumber(name) then
					name = '"' .. name .. '"'
